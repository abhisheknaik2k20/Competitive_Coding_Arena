...
int main() {
    ...
    return 0;
      ...
      ''',
    syntax: Syntax.CPP,
    tags: ['C++', 'Algorithms', 'Integer', 'Palindrome', 'Segmentation Fault'],
    upvotes: 1,
    downvotes: 1,
    views: 0,
    answers: 0,
...
Future<void> saveProblemsToFirestore() async {
  ...
  for (var problem in stflow_problems) {
    String documentId = problem.problem_title
        .replaceAll(RegExp(r'[^\w\s]'), '')
        .replaceAll(RegExp(r'\s+'), '_')
        .toLowerCase();
    if (documentId.isEmpty) {
      documentId = 'problem_${DateTime.now().millisecondsSinceEpoch}';
    }
    DocumentReference docRef = problemsCollection.doc(documentId);
    batch.set(docRef, problem.toMap(), SetOptions(merge: true));
    counter++;
    if (counter >= batchLimit) {
      try {
        await batch.commit();
        print('Batch committed. Moving to next batch...');
      } catch (e) {
